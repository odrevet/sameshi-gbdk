This is a GBDK version of sameshi https://github.com/datavorous/sameshi chess engine for GBDK. 

* The sameshi.h file is has not been modified and compile as-is with GBDK toolchain. 

* Use GBDK text example as base. 

* The main file has been adapted to input coords with the joypad instead of a keyboard, but other function also remains as-is : board print using printf and no input verification

* More an experiment than a game: Thinking is slow ! Use the turbo feature on emulator if available. 

* S parameter may be tweaked so the game would be playable on a game boy. Need more test. If playable, a "chessmaster like" interface will be made. 
